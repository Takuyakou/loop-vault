...
Phase 4.0終盤に提供された別コーパス（**外部由来MIDIに対するAI支援アノテーションseed** 100件）での結果。

| | Chord Drip | Chapter 3 Seed |
|---|---:|---:|
| canonical一致率 | 28.13% | **97.73%** |
| 検出器の語彙外 | **30.60%** | 1.0% |
...
**Chapter 3 Seedとのcanonical一致率は97.73%。** ただしSeedは教材公式ラベルでも独立専門家によるgold annotationでもないため、これを「正解率98%」とは読まない。単純コード・転回形・slash・7th・sus・diminished・altered dominant を含む**回帰テスト**として扱う。
