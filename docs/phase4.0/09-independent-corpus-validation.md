...
本コーパスは手作業注釈で、曖昧なpitch setには別解が付いており、**Phase 4.0のチューニングに一度も使っていない**。昇格判断の外部検証として使える唯一のデータである。
...
つまり **28.13% は検出器が無能であることを意味しない。** 意図的に難しく作られたデータで、かつ3割が原理的に表現できない語彙を要求している結果である。素直なコードMIDIに対する実力は98%前後にある。
