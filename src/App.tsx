...
import type { AssetType, SongIdea, Status } from "./domain/types";
import {
      ...
      {pendingDelete ? (
        <div className="fixed bottom-4 left-1/2 z-40 w-[min(92vw,440px)] -translate-x-1/2 border border-stone-700 bg-stone-900 p-3 shadow-2xl">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-stone-200">Deleting "{pendingDelete.title}"</p>
  ...
  return (
    <header className="flex flex-col gap-4 border-b border-stone-800 pb-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-normal sm:text-3xl">Focus your next loop.</h1>
      </div>
...
function HomeView({
  ...
  function completeNext(idea: SongIdea) {
    ...
    setToast("Next Action cleared. Add the next one from Detail.");
  }
  ...
  return (
    <div className="grid gap-5 py-5 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <Panel>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-semibold">Monthly finish</h2>
            </div>
          ...
          <p className="mt-3 text-sm text-stone-400">{stats.remainingDays} days left</p>
        </Panel>
          ...
          {focus.focus ? (
            <div className="mt-4 border border-stone-800 bg-stone-950 p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button className="rounded bg-stone-800 px-3 py-2 text-sm" onClick={() => openDetail(focus.focus!.id)}>Open detail</button>
                <button className="rounded bg-teal-400 px-3 py-2 text-sm font-semibold text-stone-950" onClick={() => completeNext(focus.focus!)}>Next done</button>
              </div>
            </div>
          ) : (
            <p className="mt-3 text-stone-400">No focus candidate yet. Add a Next Action to an active idea.</p>
          )}
        </Panel>
      </section>
      ...
      <aside className="space-y-5">
        ...
        <Panel>
          <h2 className="text-lg font-semibold">Needs Next Action</h2>
          <IdeaList ideas={focus.needsNextAction} openDetail={openDetail} empty="Every active idea has a next step." />
        </Panel>
        <Panel>
          <h2 className="text-lg font-semibold">Stale</h2>
          <div className="mt-3 space-y-2">
            {focus.stale.length === 0 ? <p className="text-sm text-stone-400">Nothing is over 7 days stale.</p> : null}
            {focus.stale.map((entry) => (
              <div key={entry.idea.id} className="border border-stone-800 p-3">
                ...
                <p className="mt-1 text-sm text-stone-400">{entry.idleDays} days idle {entry.suggestHold ? " · Hold suggested" : ""}</p>
                {entry.suggestHold ? (
  ...
  return (
    <div className="py-5">
      <div className="grid gap-2 border-b border-stone-800 pb-4 md:grid-cols-[1.4fr_0.7fr_0.8fr_0.8fr_0.8fr]">
        <input className={inputClass} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, chords, next action" />
        ...
        <div className="grid gap-3 py-5 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((idea) => (
            <button key={idea.id} className="border border-stone-800 bg-stone-900 p-4 text-left hover:border-teal-400" onClick={() => openDetail(idea.id)}>
              ...
              <p className="mt-2 text-sm text-stone-400">{idea.bpm ? `${idea.bpm} bpm` : "No BPM"} {idea.key ? ` · ${idea.key}` : ""}</p>
              <p className="mt-4 line-clamp-2 text-sm text-stone-300">{idea.nextAction.text || "Needs Next Action"}</p>
              {!idea.nextAction.text.trim() ? <span className="mt-3 inline-block rounded bg-amber-400 px-2 py-1 text-xs font-semibold text-stone-950">Needs Next</span> : null}
              <p className="mt-4 text-xs text-stone-500">Updated {formatDate(idea.updatedAt)}</p>
            </button>
          ))}
        </div>
      )}
...
function DetailView({
  ...
  function completeNext() {
    updateNextAction(idea.id, "", new Date());
    setNextDraft("");
    setToast("Next Action cleared. Add the next one when ready.");
  }
  ...
  function moveStatus(to: Status) {
    if (to === "abandoned" && !window.confirm("Mark this idea as abandoned?")) return;
    if (to === "hold") window.prompt("Hold reason (optional)");
    if (pipeline.includes(to) && idea.nextAction.text.trim() && to !== idea.status) {
      const keep = window.confirm("Carry the current Next Action to the next stage?");
      if (!keep) {
        updateNextAction(idea.id, "", new Date());
      }
    }
    ...
    if (!result.ok) setToast(result.error.message);
    if (result.ok && to === "done") setToast("Done. Nice finish.");
  }
  ...
  async function chooseAssetPath() {
    if (!("__TAURI_INTERNALS__" in window)) {
      setToast("File picker is available in the Tauri desktop app.");
      return;
    }
  ...
  async function openAsset(asset: Asset) {
    ...
    if (!canOpenAssetPath(asset.path)) {
      setToast("This file type can only be shown in its folder.");
      return;
    }
    if (!("__TAURI_INTERNALS__" in window)) {
      setToast("Opening files is available in the Tauri desktop app.");
      return;
    }
    try {
      ...
      setToast("Could not open the file. Marked as missing.");
    }
  }
    ...
    <div className="grid gap-5 py-5 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="space-y-5">
        <Panel>
          ...
          <button className="mt-5 rounded border border-red-500/50 px-3 py-2 text-sm text-red-200" onClick={() => requestDelete(idea)}>Delete</button>
        </Panel>
        ...
        <Panel>
          ...
          <div className="mt-3 flex gap-2">
            <button className="rounded bg-teal-400 px-3 py-2 text-sm font-semibold text-stone-950" onClick={saveNext}>Update</button>
            <button className="rounded border border-stone-700 px-3 py-2 text-sm" onClick={completeNext}>Done</button>
          </div>
          {!idea.nextAction.text.trim() ? <p className="mt-3 text-sm text-amber-200">Needs exactly one next action.</p> : null}
        </Panel>
        ...
        <Panel>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <input className={inputClass} value={idea.moods.join(", ")} onChange={(event) => updateMeta({ moods: splitList(event.target.value) })} placeholder="Moods" />
          </div>
          <textarea className={`${inputClass} mt-3 min-h-28`} value={idea.chordMemo} onChange={(event) => updateMeta({ chordMemo: event.target.value })} placeholder="Chord progression memo" />
        </Panel>
      </section>
      ...
      <section className="space-y-5">
        <Panel>
          ...
          <form className="mt-3 grid gap-2" onSubmit={addReference}>
            ...
            <button className="rounded bg-stone-800 px-3 py-2 text-sm" type="submit">Add reference</button>
          </form>
            ...
            {idea.references.map((reference, index) => (
              <div key={`${reference.title}-${index}`} className="border border-stone-800 p-3 text-sm">
                <div className="flex justify-between gap-3">
                  ...
                  <button className="text-stone-400" onClick={() => removeReference(index)}>Remove</button>
                </div>
        ...
        <Panel>
          ...
          <form className="mt-3 grid gap-2" onSubmit={addAsset}>
            <div className="grid gap-2 sm:grid-cols-[0.4fr_1fr_auto]">
              ...
              <input className={inputClass} value={assetDraft.path ?? ""} onChange={(event) => setAssetDraft({ ...assetDraft, path: event.target.value })} placeholder="Absolute path" />
              <button className="rounded border border-stone-700 px-3 py-2 text-sm" type="button" onClick={() => void chooseAssetPath()}>Pick</button>
            </div>
            ...
            <button className="rounded bg-stone-800 px-3 py-2 text-sm" type="submit">Add asset</button>
          </form>
          <div className="mt-4 space-y-2">
              ...
              <div key={asset.id} className={`border p-3 text-sm ${asset.missing ? "border-red-500/60 bg-red-950/20" : "border-stone-800"}`}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    <button className="rounded border border-stone-700 px-2 py-1" onClick={() => void openAsset(asset)}>Open</button>
                    <button className="rounded border border-stone-700 px-2 py-1" onClick={() => void showAsset(asset)}>Folder</button>
                    <button className="rounded border border-stone-700 px-2 py-1" onClick={() => removeAsset(asset.id)}>Remove</button>
                  </div>
                </div>
                <p className="mt-2 break-all text-stone-400">{asset.path || "No path"}</p>
                {!canOpenAssetPath(asset.path) && asset.path ? <p className="mt-2 text-xs text-amber-200">Open blocked by extension whitelist. Use Folder.</p> : null}
    ...
    <div className="fixed inset-0 z-40 grid place-items-center bg-black/70 px-4">
      <form className="w-full max-w-md border border-stone-700 bg-stone-900 p-5 shadow-2xl" onSubmit={submit}>
        <div className="flex items-center justify-between">
          ...
          <button type="button" className="rounded px-2 py-1 text-stone-400" onClick={onClose}>Close</button>
        </div>
        <input autoFocus className={`${inputClass} mt-4`} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Title" />
        ...
        <button className="mt-4 w-full rounded bg-teal-400 px-3 py-2 font-semibold text-stone-950" type="submit">Create</button>
      </form>
    </div>
  );
}
  ...
  return (
    <div className="grid flex-1 place-items-center py-10">
      <Panel className="w-full max-w-2xl">
        {loadStatus === "loading" || loadStatus === "idle" ? <StatusPanel title="Loading vault" body="Checking the local data file." /> : null}
        {loadStatus === "recovery" && recovery ? (
          <div>
            <StatusPanel title="Data file needs recovery" body="The damaged file was moved aside. Loop Vault did not overwrite it with empty data." />
            ...
            <div className="mt-5 space-y-2">
              ...
              )) : <p className="text-sm text-stone-400">No backups are available yet.</p>}
            </div>
          </div>
        ) : null}
        {loadStatus === "readonly" && readonly ? <StatusPanel title="Update Loop Vault" body={readonly.fileVersion ? `This data file uses fileVersion ${readonly.fileVersion}, which is newer than this app supports.` : readonly.message} /> : null}
        {loadStatus === "error" ? <StatusPanel title="Could not load vault" body={error ?? "Unknown startup error."} /> : null}
      </Panel>
    </div>
  );
}
...
function QuarantineNotice({ count }: { count: number }) {
  ...
  return (
    <div className="mt-4 border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-100">
      {count} invalid record{count === 1 ? "" : "s"} quarantined. Valid ideas loaded normally.
    </div>
  );
}
  ...
  return (
    <div className="grid min-h-96 place-items-center py-10">
      <div className="max-w-md text-center">
        ...
        <button className="mt-5 rounded bg-teal-400 px-4 py-2 font-semibold text-stone-950" onClick={openCreate}>Create idea</button>
      </div>
    </div>
  );
}
