...
export function generateBasslineExercise(snapshot: BasslineGeneratorSnapshot): BasslineGeneratorResult {
  ...
  snapshot.chords.forEach((chord, chordIndex) => {
    ...
    for (let step = 0; step < beats; step += 1) { const offset = tones[(stableHash(`${snapshot.seed}:${chordIndex}:${step}`).charCodeAt(0)) % tones.length]; events.push({ index: events.length, midiNote: Math.max(28, Math.min(55, base + offset)), startBeat: chord.startBeat + step, durationBeats: Math.min(1, chord.durationBeats - step), velocity: step === 0 ? .88 : .72, chordIndex }); }
  });
